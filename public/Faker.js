class List extends Array {
  random() {
    const index = Math.floor(Math.random() * this.length);
    return this[index];
  }
}

function randInt(min, max, step = 1) {
  return Math.floor((Math.random() * (max - min + 1) + min) / step) * step;
}

function listOf(...args) {
  if (args.length === 1) {
    return new List(1).fill(...args);
  }
  return new List(...args);
}

function isBetween(num, low, high) {
  return num >= low && num < high;
}

const data = {
  animals: listOf(
    "Aardvark",
    "Albatross",
    "Alligator",
    "Alpaca",
    "Anaconda",
    "Angelfish",
    "Ant",
    "Anteater",
    "Antelope",
    "Armadillo",
    "Baboon",
    "Badger",
    "Bald eagle",
    "Barn owl",
    "Barracuda",
    "Basilisk",
    "Bat",
    "Bear",
    "Beaver",
    "Bee",
    "Beetle",
    "Bison",
    "Blackbird",
    "Blue jay",
    "Blue whale",
    "Boa constrictor",
    "Bobcat",
    "Bonobo",
    "Buffalo",
    "Butterfly",
    "Camel",
    "Capybara",
    "Caribou",
    "Cassowary",
    "Cat",
    "Caterpillar",
    "Cheetah",
    "Chicken",
    "Chimpanzee",
    "Chinchilla",
    "Chipmunk",
    "Cobra",
    "Cockroach",
    "Cod",
    "Condor",
    "Cougar",
    "Cow",
    "Coyote",
    "Crab",
    "Crane",
    "Crocodile",
    "Crow",
    "Deer",
    "Dingo",
    "Dinosaur",
    "Dog",
    "Dolphin",
    "Donkey",
    "Dragon",
    "Dragonfly",
    "Duck",
    "Eagle",
    "Earthworm",
    "Echidna",
    "Eel",
    "Egret",
    "Elephant",
    "Elk",
    "Emu",
    "Falcon",
    "Ferret",
    "Finch",
    "Firefly",
    "Fish",
    "Flamingo",
    "Flea",
    "Fly",
    "Flying squirrel",
    "Fox",
    "Frog",
    "Gazelle",
    "Gerbil",
    "Giraffe",
    "Goat",
    "Goose",
    "Gorilla",
    "Grasshopper",
    "Groundhog",
    "Guinea pig",
    "Gull",
    "Hamster",
    "Hare",
    "Harpy eagle",
    "Hawk",
    "Hedgehog",
    "Hippopotamus",
    "Honey badger",
    "Honeybee",
    "Hornet",
    "Horse",
    "Human",
    "Hummingbird",
    "Humpback whale",
    "Hyena",
    "Ibex",
    "Ibis",
    "Iguana",
    "Impala",
    "Jackal",
    "Jaguar",
    "Jellyfish",
    "Kangaroo",
    "King cobra",
    "Kingfisher",
    "Koala",
    "Komodo dragon",
    "Kookaburra",
    "Krill",
    "Lamprey",
    "Leech",
    "Lemming",
    "Lemur",
    "Leopard",
    "Lion",
    "Llama",
    "Lobster",
    "Locust",
    "Lynx",
    "Macaque",
    "Macaw",
    "Magpie",
    "Mallard duck",
    "Manatee",
    "Mandrill",
    "Manta ray",
    "Marmot",
    "Marsupial",
    "Marten",
    "Meerkat",
    "Mink",
    "Mole",
    "Mongoose",
    "Monkey",
    "Moose",
    "Moth",
    "Mouse",
    "Mule",
    "Muskrat",
    "Narwhal",
    "Newt",
    "Nightingale",
    "Ocelot",
    "Octopus",
    "Opossum",
    "Orangutan",
    "Orca",
    "Ostrich",
    "Otter",
    "Owl",
    "Ox",
    "Panda",
    "Panther",
    "Parakeet",
    "Parrot",
    "Partridge",
    "Peacock",
    "Pelican",
    "Penguin",
    "Pheasant",
    "Pig",
    "Pigeon",
    "Pike",
    "Piranha",
    "Platypus",
    "Polar bear",
    "Pony",
    "Porcupine",
    "Porpoise",
    "Possum",
    "Prairie dog",
    "Prawn",
    "Puffin",
    "Puma",
    "Python",
    "Quail",
    "Rabbit",
    "Raccoon",
    "Ram",
    "Rat",
    "Rattlesnake",
    "Raven",
    "Red panda",
    "Reindeer",
    "Rhinoceros",
    "Roadrunner",
    "Robin",
    "Rock hyrax",
    "Rook",
    "Rooster",
    "Rottweiler",
    "Salamander",
    "Salmon",
    "Sand dollar",
    "Sandpiper",
    "Sardine",
    "Scorpion",
    "Seagull",
    "Seahorse",
    "Seal",
    "Shark",
    "Sheep",
    "Shrimp",
    "Siberian tiger",
    "Sloth",
    "Slug",
    "Snail",
    "Snake",
    "Sparrow",
    "Sperm whale",
    "Spider",
    "Sponge",
    "Squid",
    "Squirrel",
    "Starfish",
    "Stingray",
    "Stork",
    "Swallow",
    "Swan",
    "Tapir",
    "Tarantula",
    "Tarsier",
    "Termite",
    "Tiger",
    "Toad",
    "Toucan",
    "Trout",
    "Turkey",
    "Turtle",
    "Vampire bat",
    "Vulture",
    "Walrus",
    "Warthog",
    "Weasel",
    "Whale",
    "Wolf",
    "Wolverine",
    "Wombat",
    "Woodpecker",
    "Worm",
    "Wren",
    "Yak",
    "Yellowjacket",
    "Zebra"
  ),
  first_names: {
    female: listOf(
      "Abigail",
      "Adalyn",
      "Addison",
      "Adeline",
      "Ainsley",
      "Alaina",
      "Alexa",
      "Alexandra",
      "Alice",
      "Alicia",
      "Allison",
      "Alyssa",
      "Amara",
      "Amelia",
      "Amelie",
      "Amy",
      "Ana",
      "Anastasia",
      "Andrea",
      "Angelina",
      "Annabelle",
      "Aria",
      "Ariana",
      "Arianna",
      "Ariel",
      "Ashley",
      "Aubree",
      "Audrey",
      "Aurora",
      "Ava",
      "Avery",
      "Bailey",
      "Bella",
      "Bethany",
      "Bianca",
      "Brianna",
      "Brooke",
      "Caitlyn",
      "Camila",
      "Caroline",
      "Carrie",
      "Cassandra",
      "Catherine",
      "Charlotte",
      "Chloe",
      "Christina",
      "Clara",
      "Claire",
      "Clara",
      "Daniela",
      "Daphne",
      "Delilah",
      "Destiny",
      "Diana",
      "Eleanor",
      "Elena",
      "Elise",
      "Elizabeth",
      "Ella",
      "Ellie",
      "Emilia",
      "Emily",
      "Emma",
      "Emmalyn",
      "Erica",
      "Erin",
      "Eva",
      "Evelyn",
      "Faith",
      "Felicity",
      "Gabriela",
      "Gabriella",
      "Genesis",
      "Georgia",
      "Gianna",
      "Giselle",
      "Grace",
      "Gracie",
      "Hailey",
      "Haley",
      "Hannah",
      "Harper",
      "Hazel",
      "Heidi",
      "Holly",
      "Hope",
      "Isabella",
      "Isabelle",
      "Ivy",
      "Jade",
      "Jasmine",
      "Jayla",
      "Jocelyn",
      "Jordan",
      "Josie",
      "Julia",
      "Juliana",
      "Julianna",
      "Julie",
      "June",
      "Kaitlyn",
      "Katherine",
      "Katie",
      "Kayla",
      "Keira",
      "Kendall",
      "Kennedy",
      "Khloe",
      "Kimberly",
      "Kinley",
      "Kinsley",
      "Kylee",
      "Kylie",
      "Lacey",
      "Laila",
      "Lana",
      "Laura",
      "Lauren",
      "Layla",
      "Leah",
      "Leilani",
      "Lila",
      "Liliana",
      "Lillian",
      "Lilly",
      "Lily",
      "Linda",
      "Lola",
      "London",
      "Londyn",
      "Lucia",
      "Lucy",
      "Luna",
      "Lydia",
      "Madelyn",
      "Madison",
      "Makayla",
      "Makenzie",
      "Malaysia",
      "Malia",
      "Margaret",
      "Maria",
      "Mariah",
      "Marie",
      "Marilyn",
      "Marley",
      "Mary",
      "Maya",
      "Mckenzie",
      "Megan",
      "Melanie",
      "Melody",
      "Mia",
      "Michelle",
      "Mikayla",
      "Mila",
      "Molly",
      "Morgan",
      "Mya",
      "Natalia",
      "Natalie",
      "Naomi",
      "Nataly",
      "Nevaeh",
      "Nina",
      "Noelle",
      "Nora",
      "Olivia",
      "Paige",
      "Paisley",
      "Payton",
      "Penelope",
      "Peyton",
      "Phoebe",
      "Piper",
      "Quinn",
      "Rachel",
      "Raelynn",
      "Reagan",
      "Rebecca",
      "Riley",
      "River",
      "Rose",
      "Rowan",
      "Ruby",
      "Rylee",
      "Sabrina",
      "Sadie",
      "Samantha",
      "Sara",
      "Sarah",
      "Savannah",
      "Scarlett",
      "Selena",
      "Serena",
      "Shelby",
      "Sienna",
      "Sierra",
      "Skye",
      "Skylar",
      "Sofia",
      "Sophia",
      "Sophie",
      "Stella",
      "Stephanie",
      "Summer",
      "Sydney",
      "Talia",
      "Tatiana",
      "Taylor",
      "Teagan",
      "Tessa",
      "Thalia",
      "Thea",
      "Trinity",
      "Valentina",
      "Valerie",
      "Vanessa",
      "Victoria",
      "Violet",
      "Vivian",
      "Willow",
      "Winter",
      "Ximena",
      "Yara",
      "Yaretzi",
      "Yasmin",
      "Zara",
      "Zoe"
    ),
    male: listOf(
      "Aaron",
      "Adam",
      "Adrian",
      "Aiden",
      "Alan",
      "Albert",
      "Alec",
      "Alex",
      "Alexander",
      "Alfred",
      "Andrew",
      "Anthony",
      "Archer",
      "Arthur",
      "Asher",
      "Austin",
      "Avery",
      "Barry",
      "Benjamin",
      "Blake",
      "Braden",
      "Bradley",
      "Brandon",
      "Brayden",
      "Brendan",
      "Brent",
      "Brett",
      "Brian",
      "Bruce",
      "Bryan",
      "Caleb",
      "Calvin",
      "Cameron",
      "Carl",
      "Carson",
      "Carter",
      "Casey",
      "Charles",
      "Charlie",
      "Chase",
      "Chris",
      "Christian",
      "Christopher",
      "Clayton",
      "Cody",
      "Colin",
      "Collin",
      "Connor",
      "Cooper",
      "Corey",
      "Craig",
      "Curtis",
      "Dale",
      "Dallas",
      "Dalton",
      "Damian",
      "Daniel",
      "Dante",
      "Darius",
      "Darnell",
      "Darren",
      "David",
      "Davis",
      "Dean",
      "Declan",
      "Dennis",
      "Derek",
      "Derrick",
      "Devin",
      "Dillon",
      "Dominic",
      "Donovan",
      "Dorian",
      "Douglas",
      "Drew",
      "Dwayne",
      "Dylan",
      "Easton",
      "Edgar",
      "Edison",
      "Eduardo",
      "Edward",
      "Edwin",
      "Eli",
      "Elias",
      "Elijah",
      "Elliot",
      "Elliott",
      "Elon",
      "Emiliano",
      "Emmanuel",
      "Eric",
      "Erick",
      "Erik",
      "Ernest",
      "Ethan",
      "Eugene",
      "Evan",
      "Everett",
      "Ezekiel",
      "Fabian",
      "Felix",
      "Fernando",
      "Finn",
      "Francis",
      "Frank",
      "Franklin",
      "Fred",
      "Frederick",
      "Gage",
      "Garrett",
      "Gary",
      "Gavin",
      "George",
      "Gerald",
      "Gilbert",
      "Giovanni",
      "Glen",
      "Gordon",
      "Graham",
      "Grant",
      "Gregory",
      "Greyson",
      "Griffin",
      "Gunnar",
      "Gustavo",
      "Hank",
      "Harold",
      "Harrison",
      "Harry",
      "Hayden",
      "Heath",
      "Henry",
      "Hudson",
      "Hunter",
      "Ian",
      "Isaac",
      "Isaiah",
      "Ivan",
      "Jack",
      "Jackson",
      "Jacob",
      "Jaden",
      "Jake",
      "James",
      "Jared",
      "Jason",
      "Jasper",
      "Jay",
      "Jayden",
      "Jeff",
      "Jeffrey",
      "Jeremiah",
      "Jeremy",
      "Jerome",
      "Jerry",
      "Jesse",
      "Jesus",
      "Joel",
      "John",
      "Johnny",
      "Jonah",
      "Jonathan",
      "Jordan",
      "Joseph",
      "Joshua",
      "Josiah",
      "Juan",
      "Judah",
      "Julian",
      "Julio",
      "Justin",
      "Kaden",
      "Kai",
      "Kaleb",
      "Kameron",
      "Karl",
      "Karter",
      "Kayden",
      "Keith",
      "Kelly",
      "Kelvin",
      "Ken",
      "Kenneth",
      "Kent",
      "Kevin",
      "Kieran",
      "King",
      "Kobe",
      "Kody",
      "Kolton",
      "Kris",
      "Kristopher",
      "Kyle",
      "Kyler",
      "Landon",
      "Larry",
      "Lawrence",
      "Leandro",
      "Lee",
      "Leo",
      "Leonardo",
      "Leonel",
      "Liam",
      "Lincoln",
      "Logan",
      "Lorenzo",
      "Louie",
      "Louis",
      "Lucas",
      "Luis",
      "Luke",
      "Maddox",
      "Malachi",
      "Malcolm",
      "Manuel",
      "Marc",
      "Marco",
      "Marcus",
      "Mario",
      "Mark",
      "Marshall",
      "Martin",
      "Marvin",
      "Mason",
      "Mateo",
      "Mathew",
      "Matias",
      "Matthew",
      "Maurice",
      "Maverick",
      "Max",
      "Maximilian",
      "Maximus",
      "Mekhi",
      "Melvin",
      "Micah",
      "Michael",
      "Miguel",
      "Mike",
      "Miles",
      "Misael",
      "Mitchell",
      "Mohamed",
      "Moises",
      "Nash",
      "Nathan",
      "Nathaniel",
      "Neal",
      "Neil",
      "Nelson",
      "Nestor",
      "Nicholas",
      "Nick",
      "Nicolas",
      "Noah",
      "Nolan",
      "Oliver",
      "Omar",
      "Orlando",
      "Oscar",
      "Owen",
      "Pablo",
      "Parker",
      "Patrick",
      "Paul",
      "Peter",
      "Peyton",
      "Philip",
      "Phillip",
      "Phoenix",
      "Preston",
      "Quentin",
      "Rafael",
      "Ralph",
      "Ramiro",
      "Ramsey",
      "Randal",
      "Randall",
      "Randy",
      "Raphael",
      "Ray",
      "Raymond",
      "Reece",
      "Reed",
      "Reginald",
      "Rene",
      "Reuben",
      "Rex",
      "Rhett",
      "Ricardo",
      "Richard",
      "Rick",
      "Rickey",
      "Ricky",
      "Riley",
      "River",
      "Roberto",
      "Robin",
      "Rocco",
      "Rocky",
      "Rodney",
      "Rodolfo",
      "Roger",
      "Roland",
      "Roman",
      "Ronald",
      "Ronan",
      "Ronnie",
      "Rory",
      "Ross",
      "Rowan",
      "Roy",
      "Ruben",
      "Rudy",
      "Russell",
      "Ryan",
      "Ryder",
      "Ryker",
      "Sage",
      "Salvador",
      "Sam",
      "Samuel",
      "Santiago",
      "Santos",
      "Saul",
      "Scott",
      "Seamus",
      "Sean",
      "Sebastian",
      "Sergio",
      "Seth",
      "Shane",
      "Shaun",
      "Shawn",
      "Silas",
      "Simon",
      "Skyler",
      "Solomon",
      "Spencer",
      "Stanley",
      "Stefan",
      "Stephen",
      "Steve",
      "Steven",
      "Stewart",
      "Stuart",
      "Sullivan",
      "Sydney",
      "Tanner",
      "Tate",
      "Taylor",
      "Terrance",
      "Terrell",
      "Terry",
      "Theodore",
      "Thomas",
      "Timothy",
      "Tobias",
      "Todd",
      "Tom",
      "Tommy",
      "Tony",
      "Trace",
      "Travis",
      "Trent",
      "Trevor",
      "Trey",
      "Tristan",
      "Troy",
      "Tucker",
      "Ty",
      "Tyler",
      "Tyrone",
      "Ulises",
      "Uriel",
      "Valentin",
      "Valentino",
      "Van",
      "Vance",
      "Victor",
      "Vincent",
      "Wade",
      "Walker",
      "Wallace",
      "Walter",
      "Warren",
      "Wayne",
      "Wesley",
      "Weston",
      "Wilfredo",
      "Will",
      "William",
      "Willie",
      "Wilson",
      "Winston",
      "Xander",
      "Xavier",
      "Yadiel",
      "Yahir",
      "Yosef",
      "Yusuf",
      "Zachariah",
      "Zachary",
      "Zaiden",
      "Zander",
      "Zane",
      "Zechariah",
      "Zion"
    ),
  },
  last_names: listOf(
    "Adams",
    "Allen",
    "Anderson",
    "Bailey",
    "Baker",
    "Barnes",
    "Bell",
    "Bennett",
    "Bishop",
    "Black",
    "Bryant",
    "Burke",
    "Burns",
    "Butler",
    "Campbell",
    "Carpenter",
    "Carter",
    "Chapman",
    "Clark",
    "Cole",
    "Collins",
    "Cook",
    "Cooper",
    "Cox",
    "Crawford",
    "Cruz",
    "Cunningham",
    "Daniels",
    "Davis",
    "Diaz",
    "Dixon",
    "Douglas",
    "Duncan",
    "Edwards",
    "Ellis",
    "Evans",
    "Ferguson",
    "Fisher",
    "Fleming",
    "Ford",
    "Foster",
    "Fowler",
    "Franklin",
    "Freeman",
    "Garcia",
    "Gardner",
    "Gibson",
    "Gomez",
    "Gonzales",
    "Gonzalez",
    "Graham",
    "Grant",
    "Gray",
    "Green",
    "Greene",
    "Griffin",
    "Hall",
    "Hamilton",
    "Hansen",
    "Hanson",
    "Harper",
    "Harris",
    "Harrison",
    "Hart",
    "Harvey",
    "Hawkins",
    "Hayes",
    "Henderson",
    "Henry",
    "Hicks",
    "Hill",
    "Hoffman",
    "Holmes",
    "Howard",
    "Howell",
    "Hughes",
    "Hunt",
    "Hunter",
    "Jackson",
    "Jacobs",
    "James",
    "Jenkins",
    "Jensen",
    "Johnson",
    "Jones",
    "Jordan",
    "Kelley",
    "Kelly",
    "Kennedy",
    "Kim",
    "King",
    "Knight",
    "Lane",
    "Lawrence",
    "Lawson",
    "Lee",
    "Lewis",
    "Little",
    "Long",
    "Lopez",
    "Lucas",
    "Lynch",
    "Lyons",
    "Mack",
    "Maldonado",
    "Marshall",
    "Martin",
    "Martinez",
    "Mason",
    "Matthews",
    "May",
    "Mccarthy",
    "Mcdonald",
    "Medina",
    "Mendez",
    "Mendoza",
    "Meyer",
    "Miller",
    "Mills",
    "Mitchell",
    "Moore",
    "Morgan",
    "Morris",
    "Morrison",
    "Murphy",
    "Murray",
    "Myers",
    "Nelson",
    "Newman",
    "Nguyen",
    "Nichols",
    "Norton",
    "Nunez",
    "O'brien",
    "Oliver",
    "Ortiz",
    "Owens",
    "Palmer",
    "Parker",
    "Patterson",
    "Payne",
    "Pearson",
    "Perez",
    "Perkins",
    "Perry",
    "Peters",
    "Peterson",
    "Phillips",
    "Pierce",
    "Porter",
    "Powell",
    "Price",
    "Ramirez",
    "Ramos",
    "Ray",
    "Reed",
    "Reid",
    "Reyes",
    "Reynolds",
    "Rhodes",
    "Rice",
    "Richards",
    "Richardson",
    "Riley",
    "Rivera",
    "Roberts",
    "Robertson",
    "Robinson",
    "Rodriguez",
    "Rogers",
    "Romero",
    "Rose",
    "Ross",
    "Ruiz",
    "Russell",
    "Ryan",
    "Sanchez",
    "Sanders",
    "Sandoval",
    "Santos",
    "Schmidt",
    "Scott",
    "Shaw",
    "Silva",
    "Simmons",
    "Simpson",
    "Smith",
    "Snyder",
    "Soto",
    "Spencer",
    "Stanley",
    "Step",
    "Stevens",
    "Stewart",
    "Stone",
    "Sullivan",
    "Taylor",
    "Thomas",
    "Thompson",
    "Torres",
    "Turner",
    "Vargas",
    "Vasquez",
    "Walker",
    "Wallace",
    "Ward",
    "Warren",
    "Washington",
    "Watkins",
    "Watson",
    "Weaver",
    "Webb",
    "Welch",
    "Wells",
    "West",
    "Wheeler",
    "White",
    "Williams",
    "Williamson",
    "Willis",
    "Wilson",
    "Wong",
    "Wood",
    "Woods",
    "Wright",
    "Wyatt",
    "Young"
  ),
  email_domains: listOf(
    "protonmail.com",
    "mail.com",
    "outlook.com",
    "icloud.com",
    "yahoo.com",
    "hotmail.com",
    "gmx.com",
    "tutanota.com",
    "zoho.com",
    "aol.com",
    "yandex.com",
    "live.com",
    "inbox.com",
    "mail.ru",
    "fastmail.com",
    "rocketmail.com",
    "microsoft.com",
    "me.com",
    "ymail.com",
    "lavabit.com",
    "runbox.com",
    "hushmail.com",
    "startmail.com",
    "icloud.com",
    "mailinator.com",
    "gmx.us",
    "comcast.net",
    "netcourrier.com",
    "cox.net",
    "safe-mail.net",
    "pobox.com",
    "protonmail.ch",
    "abv.bg",
    "optonline.net",
    "gmx.de",
    "juno.com",
    "lycos.com",
    "earthlink.net",
    "ig.com.br",
    "libero.it",
    "seznam.cz",
    "virginmedia.com",
    "windstream.net",
    "shaw.ca",
    "sympatico.ca",
    "wowway.com",
    "bellsouth.net",
    "charter.net"
  ),
  colours: listOf(
    {
      name: "Pink",
      hex: "#FFC0CB",
    },
    {
      name: "LightPink",
      hex: "#FFB6C1",
    },
    {
      name: "HotPink",
      hex: "#FF69B4",
    },
    {
      name: "DeepPink",
      hex: "#FF1493",
    },
    {
      name: "PaleVioletRed",
      hex: "#DB7093",
    },
    {
      name: "MediumVioletRed",
      hex: "#C71585",
    },
    {
      name: "Lavender",
      hex: "#E6E6FA",
    },
    {
      name: "Thistle",
      hex: "#D8BFD8",
    },
    {
      name: "Plum",
      hex: "#DDA0DD",
    },
    {
      name: "Orchid",
      hex: "#DA70D6",
    },
    {
      name: "Violet",
      hex: "#EE82EE",
    },
    {
      name: "Fuchsia",
      hex: "#FF00FF",
    },
    {
      name: "Magenta",
      hex: "#FF00FF",
    },
    {
      name: "MediumOrchid",
      hex: "#BA55D3",
    },
    {
      name: "DarkOrchid",
      hex: "#9932CC",
    },
    {
      name: "DarkViolet",
      hex: "#9400D3",
    },
    {
      name: "BlueViolet",
      hex: "#8A2BE2",
    },
    {
      name: "DarkMagenta",
      hex: "#8B008B",
    },
    {
      name: "Purple",
      hex: "#800080",
    },
    {
      name: "MediumPurple",
      hex: "#9370DB",
    },
    {
      name: "MediumSlateBlue",
      hex: "#7B68EE",
    },
    {
      name: "SlateBlue",
      hex: "#6A5ACD",
    },
    {
      name: "DarkSlateBlue",
      hex: "#483D8B",
    },
    {
      name: "RebeccaPurple",
      hex: "#663399",
    },
    {
      name: "Indigo",
      hex: "#4B0082",
    },
    {
      name: "LightSalmon",
      hex: "#FFA07A",
    },
    {
      name: "Salmon",
      hex: "#FA8072",
    },
    {
      name: "DarkSalmon",
      hex: "#E9967A",
    },
    {
      name: "LightCoral",
      hex: "#F08080",
    },
    {
      name: "IndianRed ",
      hex: "#CD5C5C",
    },
    {
      name: "Crimson",
      hex: "#DC143C",
    },
    {
      name: "Red",
      hex: "#FF0000",
    },
    {
      name: "FireBrick",
      hex: "#B22222",
    },
    {
      name: "DarkRed",
      hex: "#8B0000",
    },
    {
      name: "Orange",
      hex: "#FFA500",
    },
    {
      name: "DarkOrange",
      hex: "#FF8C00",
    },
    {
      name: "Coral",
      hex: "#FF7F50",
    },
    {
      name: "Tomato",
      hex: "#FF6347",
    },
    {
      name: "OrangeRed",
      hex: "#FF4500",
    },
    {
      name: "Gold",
      hex: "#FFD700",
    },
    {
      name: "Yellow",
      hex: "#FFFF00",
    },
    {
      name: "LightYellow",
      hex: "#FFFFE0",
    },
    {
      name: "LemonChiffon",
      hex: "#FFFACD",
    },
    {
      name: "LightGoldenRodYellow",
      hex: "#FAFAD2",
    },
    {
      name: "PapayaWhip",
      hex: "#FFEFD5",
    },
    {
      name: "Moccasin",
      hex: "#FFE4B5",
    },
    {
      name: "PeachPuff",
      hex: "#FFDAB9",
    },
    {
      name: "PaleGoldenRod",
      hex: "#EEE8AA",
    },
    {
      name: "Khaki",
      hex: "#F0E68C",
    },
    {
      name: "DarkKhaki",
      hex: "#BDB76B",
    },
    {
      name: "GreenYellow",
      hex: "#ADFF2F",
    },
    {
      name: "Chartreuse",
      hex: "#7FFF00",
    },
    {
      name: "LawnGreen",
      hex: "#7CFC00",
    },
    {
      name: "Lime",
      hex: "#00FF00",
    },
    {
      name: "LimeGreen",
      hex: "#32CD32",
    },
    {
      name: "PaleGreen",
      hex: "#98FB98",
    },
    {
      name: "LightGreen",
      hex: "#90EE90",
    },
    {
      name: "MediumSpringGreen",
      hex: "#00FA9A",
    },
    {
      name: "SpringGreen",
      hex: "#00FF7F",
    },
    {
      name: "MediumSeaGreen",
      hex: "#3CB371",
    },
    {
      name: "SeaGreen",
      hex: "#2E8B57",
    },
    {
      name: "ForestGreen",
      hex: "#228B22",
    },
    {
      name: "Green",
      hex: "#008000",
    },
    {
      name: "DarkGreen",
      hex: "#006400",
    },
    {
      name: "YellowGreen",
      hex: "#9ACD32",
    },
    {
      name: "OliveDrab",
      hex: "#6B8E23",
    },
    {
      name: "DarkOliveGreen",
      hex: "#556B2F",
    },
    {
      name: "MediumAquaMarine",
      hex: "#66CDAA",
    },
    {
      name: "DarkSeaGreen",
      hex: "#8FBC8F",
    },
    {
      name: "LightSeaGreen",
      hex: "#20B2AA",
    },
    {
      name: "DarkCyan",
      hex: "#008B8B",
    },
    {
      name: "Teal",
      hex: "#008080",
    },
    {
      name: "Aqua",
      hex: "#00FFFF",
    },
    {
      name: "Cyan",
      hex: "#00FFFF",
    },
    {
      name: "LightCyan",
      hex: "#E0FFFF",
    },
    {
      name: "PaleTurquoise",
      hex: "#AFEEEE",
    },
    {
      name: "Aquamarine",
      hex: "#7FFFD4",
    },
    {
      name: "Turquoise",
      hex: "#40E0D0",
    },
    {
      name: "MediumTurquoise",
      hex: "#48D1CC",
    },
    {
      name: "DarkTurquoise",
      hex: "#00CED1",
    },
    {
      name: "CadetBlue",
      hex: "#5F9EA0",
    },
    {
      name: "SteelBlue",
      hex: "#4682B4",
    },
    {
      name: "LightSteelBlue",
      hex: "#B0C4DE",
    },
    {
      name: "LightBlue",
      hex: "#ADD8E6",
    },
    {
      name: "PowderBlue",
      hex: "#B0E0E6",
    },
    {
      name: "LightSkyBlue",
      hex: "#87CEFA",
    },
    {
      name: "SkyBlue",
      hex: "#87CEEB",
    },
    {
      name: "CornflowerBlue",
      hex: "#6495ED",
    },
    {
      name: "DeepSkyBlue",
      hex: "#00BFFF",
    },
    {
      name: "DodgerBlue",
      hex: "#1E90FF",
    },
    {
      name: "RoyalBlue",
      hex: "#4169E1",
    },
    {
      name: "Blue",
      hex: "#0000FF",
    },
    {
      name: "MediumBlue",
      hex: "#0000CD",
    },
    {
      name: "DarkBlue",
      hex: "#00008B",
    },
    {
      name: "Navy",
      hex: "#000080",
    },
    {
      name: "MidnightBlue",
      hex: "#191970",
    },
    {
      name: "Cornsilk",
      hex: "#FFF8DC",
    },
    {
      name: "BlanchedAlmond",
      hex: "#FFEBCD",
    },
    {
      name: "Bisque",
      hex: "#FFE4C4",
    },
    {
      name: "NavajoWhite",
      hex: "#FFDEAD",
    },
    {
      name: "Wheat",
      hex: "#F5DEB3",
    },
    {
      name: "BurlyWood",
      hex: "#DEB887",
    },
    {
      name: "Tan",
      hex: "#D2B48C",
    },
    {
      name: "RosyBrown",
      hex: "#BC8F8F",
    },
    {
      name: "SandyBrown",
      hex: "#F4A460",
    },
    {
      name: "GoldenRod",
      hex: "#DAA520",
    },
    {
      name: "DarkGoldenRod",
      hex: "#B8860B",
    },
    {
      name: "Peru",
      hex: "#CD853F",
    },
    {
      name: "Chocolate",
      hex: "#D2691E",
    },
    {
      name: "Olive",
      hex: "#808000",
    },
    {
      name: "SaddleBrown",
      hex: "#8B4513",
    },
    {
      name: "Sienna",
      hex: "#A0522D",
    },
    {
      name: "Brown",
      hex: "#A52A2A",
    },
    {
      name: "Maroon",
      hex: "#800000",
    },
    {
      name: "White",
      hex: "#FFFFFF",
    },
    {
      name: "Snow",
      hex: "#FFFAFA",
    },
    {
      name: "HoneyDew",
      hex: "#F0FFF0",
    },
    {
      name: "MintCream",
      hex: "#F5FFFA",
    },
    {
      name: "Azure",
      hex: "#F0FFFF",
    },
    {
      name: "AliceBlue",
      hex: "#F0F8FF",
    },
    {
      name: "GhostWhite",
      hex: "#F8F8FF",
    },
    {
      name: "WhiteSmoke",
      hex: "#F5F5F5",
    },
    {
      name: "SeaShell",
      hex: "#FFF5EE",
    },
    {
      name: "Beige",
      hex: "#F5F5DC",
    },
    {
      name: "OldLace",
      hex: "#FDF5E6",
    },
    {
      name: "FloralWhite",
      hex: "#FFFAF0",
    },
    {
      name: "Ivory",
      hex: "#FFFFF0",
    },
    {
      name: "AntiqueWhite",
      hex: "#FAEBD7",
    },
    {
      name: "Linen",
      hex: "#FAF0E6",
    },
    {
      name: "LavenderBlush",
      hex: "#FFF0F5",
    },
    {
      name: "MistyRose",
      hex: "#FFE4E1",
    },
    {
      name: "Gainsboro",
      hex: "#DCDCDC",
    },
    {
      name: "LightGray",
      hex: "#D3D3D3",
    },
    {
      name: "Silver",
      hex: "#C0C0C0",
    },
    {
      name: "DarkGray",
      hex: "#A9A9A9",
    },
    {
      name: "DimGray",
      hex: "#696969",
    },
    {
      name: "Gray",
      hex: "#808080",
    },
    {
      name: "LightSlateGray",
      hex: "#778899",
    },
    {
      name: "SlateGray",
      hex: "#708090",
    },
    {
      name: "DarkSlateGray",
      hex: "#2F4F4F",
    },
    {
      name: "Black",
      hex: "#000000",
    }
  ),
  cities: listOf(
    "Westhaven",
    "Windingo",
    "Meadowville",
    "Harmonia",
    "Highcroft",
    "Sunnymead",
    "Oakview",
    "Bridgestone",
    "Cedarville",
    "Clearview",
    "Springfield",
    "Mapleton",
    "Riverside",
    "Glendale",
    "Waverly",
    "Pinecrest",
    "Belmont",
    "Crestview",
    "Pleasantville",
    "Hillside",
    "Rockville",
    "Greenfield",
    "Southport",
    "Lakeview",
    "Fairview",
    "Briarwood",
    "Magnolia",
    "Woodland",
    "Parkview",
    "Brookfield",
    "Autumn Ridge",
    "Northfield",
    "Fieldstone",
    "Edmonton",
    "Golden Meadow",
    "Deerwood",
    "Windsor",
    "Silver Lake",
    "Summerfield",
    "Riverstone",
    "Meadowlark",
    "Whispering Pines",
    "Bellefonte",
    "Sunrise",
    "Holly Springs",
    "Kingswood",
    "Harbor View",
    "Bayview",
    "Springdale",
    "Fairfield",
    "Fox Run",
    "Greenbrier",
    "Meadowbrook",
    "Cypress Point",
    "Saddlebrook",
    "Fountain Valley",
    "River Oaks",
    "Emerald Hills",
    "Heritage Hills",
    "Rolling Hills",
    "Willow Creek",
    "Northgate",
    "Hickory Ridge",
    "Laurelwood",
    "Westgate",
    "Southwood",
    "Cherry Hill",
    "Pinehurst",
    "Wildwood",
    "Westfield",
    "Woodridge",
    "Highland Park",
    "Stonebridge",
    "Holly Hills",
    "Crystal Lake",
    "River Bend",
    "Creekwood",
    "Hillcrest",
    "Morningside",
    "Oakridge",
    "Countryside",
    "Woodbine",
    "Briarcliff",
    "Maplewood",
    "Bayside",
    "Seaview",
    "Northwood",
    "Rosewood",
    "Lakeshore",
    "Shady Grove",
    "Pleasant Hill",
    "Glenwood",
    "Wyndham Vale",
    "Mulgrave"
  ),
  states: listOf(
    "Alberta",
    "Andalusia",
    "Angus",
    "Aragon",
    "Arkhangelsk",
    "Attica",
    "Baden-Württemberg",
    "Bahia",
    "Bavaria",
    "Beijing",
    "Bihar",
    "Bohol",
    "Bolzano-Bozen",
    "Brandenburg",
    "British Columbia",
    "Buenos Aires",
    "Burgenland",
    "California",
    "Campania",
    "Cantabria",
    "Catalonia",
    "Central Java",
    "Central Kalimantan",
    "Chiang Mai",
    "Chihuahua",
    "Chubut",
    "Coahuila",
    "Colorado",
    "Connecticut",
    "Cordoba",
    "Crete",
    "Cuyo",
    "Dalarna",
    "Delaware",
    "District of Columbia",
    "East Java",
    "Eastern Cape",
    "Emilia-Romagna",
    "England",
    "Flanders",
    "Friuli-Venezia Giulia",
    "Galicia",
    "Gauteng",
    "Georgia",
    "Groningen",
    "Guanajuato",
    "Guangdong",
    "Gyeongsangbuk-do",
    "Hamburg",
    "Haryana",
    "Hawaii",
    "Hesse",
    "Hidalgo",
    "Hokkaido",
    "Hungary",
    "Idaho",
    "Ile-de-France",
    "Illinois",
    "Indiana",
    "Ionian Islands",
    "Iowa",
    "Irian Jaya",
    "Jalisco",
    "Jharkhand",
    "Kamchatka",
    "Kansas",
    "Kentucky",
    "Kerala",
    "Kharkiv Oblast",
    "Krasnoyarsk",
    "Kumamoto",
    "Kwazulu-Natal",
    "Languedoc-Roussillon",
    "Lazio",
    "Limburg",
    "Limpopo",
    "Lombardy",
    "Louisiana",
    "Lower Saxony",
    "Madhya Pradesh",
    "Madrid",
    "Maine",
    "Manitoba",
    "Maritime",
    "Maryland",
    "Massachusetts",
    "Mato Grosso do Sul",
    "Mazovia",
    "Michigan",
    "Milwood",
    "Misiones",
    "Mississippi",
    "Missouri",
    "Molise",
    "Montana",
    "Morelos",
    "Moscow Oblast",
    "Mpumalanga",
    "Murcia",
    "Victoria"
  ),
  address: {
    street_names: listOf(
      "Elmwood",
      "Oakwood",
      "Pinehurst",
      "Maplewood",
      "Birchwood",
      "Cedarwood",
      "Hollyhock",
      "Meadowview",
      "Fairway",
      "Briarwood",
      "Cherrywood",
      "Willowbrook",
      "Redwood",
      "Woodland",
      "Greenwood",
      "Juniper",
      "Magnolia",
      "Hawthorne",
      "Cypress",
      "Laurel",
      "Acorn",
      "Dogwood",
      "Peachtree",
      "Poplar",
      "Locust",
      "Chestnut",
      "Sycamore",
      "Spruce",
      "Aspen",
      "Beechwood",
      "Buckeye",
      "Cactus",
      "Camellia",
      "Clover",
      "Cottonwood",
      "Daisy",
      "Eucalyptus",
      "Fern",
      "Gardenia",
      "Ginkgo",
      "Heather",
      "Holly",
      "Ivy",
      "Jasmine",
      "Kingswood",
      "Lilac",
      "Lily",
      "Marigold",
      "Nectarine",
      "Olive",
      "Orchid",
      "Palm",
      "Pear",
      "Peony",
      "Persimmon",
      "Primrose",
      "Redbud",
      "Rose",
      "Sage",
      "Sassafras",
      "Tulip",
      "Vine",
      "Walnut",
      "Water Lily",
      "Yew",
      "Zinnia",
      "Aster",
      "Basil",
      "Butternut",
      "Chives",
      "Daffodil",
      "Elderberry",
      "Fennel",
      "Grapefruit",
      "Hazelnut",
      "Jicama",
      "Kale",
      "Lemon",
      "Mango",
      "Nutmeg",
      "Onion",
      "Parsley",
      "Quince",
      "Radish",
      "Sesame",
      "Thyme",
      "Umbrella",
      "Vanilla",
      "Wisteria",
      "Xigua",
      "Yellowwood",
      "Zucchini"
    ),
    street_types: listOf(
      "Street",
      "Road",
      "Avenue",
      "Boulevard",
      "Drive",
      "Court",
      "Way",
      "Lane",
      "Place",
      "Terrace",
      "Circle",
      "Parkway",
      "Crescent",
      "Square",
      "Highway",
      "Path",
      "Trace",
      "Trail",
      "Park",
      "Commons",
      "Close",
      "Alley",
      "Gate",
      "Grove",
      "Mews",
      "Row",
      "Gardens",
      "Hill",
      "Heights",
      "Pass",
      "Point",
      "Ridge",
      "Run",
      "Place",
      "Pathway",
      "Loop",
      "Ramp",
      "Gate",
      "Walk",
      "Wharf",
      "Quay",
      "Harbor",
      "Esplanade",
      "Strand",
      "Place",
      "Landing",
      "Boardwalk",
      "Crossing"
    ),
  },
  cardinal_direction: listOf(
    "North",
    "South",
    "East",
    "West",
    "North East",
    "North West",
    "South East",
    "South West"
  ),
  vehicle_types: listOf(
    "Convertible",
    "Coupe",
    "Hatchback",
    "Sedan",
    "Station Wagon",
    "SUV",
    "Minivan",
    "Pickup Truck",
    "Truck",
    "Van",
    "Motorcycle",
    "Moped",
    "Scooter",
    "ATV",
    "RV",
    "Bus",
    "Ambulance",
    "Fire Truck",
    "Police Car",
    "Race Car",
    "Go-Kart",
    "Golf Cart",
    "Jet Ski",
    "Boat",
    "Yacht",
    "Helicopter",
    "Airplane",
    "Blimp",
    "Hot Air Balloon",
    "Space Shuttle",
    "Rocket",
    "Submarine",
    "Segway",
    "Skateboard",
    "Bicycle",
    "Tricycle",
    "Unicycle",
    "Roller Skates",
    "Ice Skates",
    "Ski-Doo",
    "Snowmobile",
    "Snowboard",
    "Ski",
    "Hang Glider",
    "Paraglider",
    "Kiteboard",
    "Wakeboard",
    "Surfboard"
  ),
  language: {
    adjectives: listOf(
      "Cheerful",
      "Delicious",
      "Lively",
      "Gorgeous",
      "Vivacious",
      "Exciting",
      "Cozy",
      "Amusing",
      "Radiant",
      "Energetic",
      "Fantastic",
      "Fierce",
      "Charming",
      "Sizzling",
      "Soothing",
      "Spectacular",
      "Peaceful",
      "Adventurous",
      "Marvelous",
      "Exquisite",
      "Gleaming",
      "Blushing",
      "Dazzling",
      "Enchanting",
      "Glistening",
      "Gracious",
      "Stylish",
      "Witty",
      "Graceful",
      "Incredible",
      "Intriguing",
      "Majestic",
      "Merry",
      "Mystical",
      "Playful",
      "Pleasant",
      "Refreshing",
      "Serene",
      "Splendid",
      "Thrilling",
      "Thoughtful",
      "Vibrant",
      "Wondrous",
      "Chic",
      "Brilliant",
      "Captivating",
      "Clever",
      "Curious",
      "Daring",
      "Dazzling",
      "Determined",
      "Dynamic",
      "Ecstatic",
      "Elegant",
      "Empowered",
      "Fascinating",
      "Fearless",
      "Fierce",
      "Flawless",
      "Friendly",
      "Gentle",
      "Glamorous",
      "Graceful",
      "Happy",
      "Harmonious",
      "Healthy",
      "Heroic",
      "Inquisitive",
      "Inspiring",
      "Joyful",
      "Lively",
      "Magical",
      "Majestic",
      "Mighty",
      "Noble",
      "Optimistic",
      "Passionate",
      "Peaceful",
      "Perfect",
      "Powerful",
      "Radiant",
      "Relaxed",
      "Resilient",
      "Sensational",
      "Skillful",
      "Smart",
      "Sparkling",
      "Splendid",
      "Stunning",
      "Successful",
      "Sunny",
      "Superb",
      "Terrific",
      "Trustworthy",
      "Valiant",
      "Vibrant"
    ),
    nouns: listOf(
      "Car",
      "Tree",
      "Beach",
      "Piano",
      "Sailboat",
      "Guitar",
      "Mountain",
      "Bird",
      "Computer",
      "Camera",
      "Cup",
      "Dog",
      "Cat",
      "Pizza",
      "Book",
      "Phone",
      "Chair",
      "Cloud",
      "Ocean",
      "River",
      "Rain",
      "Sun",
      "Moon",
      "Star",
      "Plane",
      "Train",
      "Ship",
      "Bicycle",
      "Motorcycle",
      "Bus",
      "Building",
      "Bridge",
      "Road",
      "Clock",
      "Watch",
      "Key",
      "Lock",
      "Door",
      "Window",
      "Table",
      "Bed",
      "Shoe",
      "Hat",
      "Jacket",
      "Umbrella",
      "Garden",
      "Pencil",
      "Pen",
      "Paper",
      "Painting",
      "Sculpture",
      "Plant",
      "Flower",
      "Fruit",
      "Vegetable",
      "Bottle",
      "Glass",
      "Knife",
      "Fork",
      "Spoon",
      "Plate",
      "Bowl",
      "Napkin",
      "Towel",
      "Soap",
      "Shampoo",
      "Toothbrush",
      "Toothpaste",
      "Mirror",
      "Candle",
      "Fireplace",
      "Television",
      "Radio",
      "Movie",
      "Music",
      "Dance",
      "Art",
      "Sport",
      "Game",
      "Toy",
      "Baby",
      "Child",
      "Parent",
      "Grandparent",
      "Friend",
      "Teacher",
      "Student",
      "Doctor",
      "Lawyer",
      "Engineer",
      "Scientist",
      "Writer",
      "Musician",
      "Actor",
      "Artist",
      "Chef",
      "Photographer"
    ),
    verbs: listOf(
      "Run",
      "Jump",
      "Swim",
      "Dance",
      "Sing",
      "Laugh",
      "Smile",
      "Read",
      "Write",
      "Draw",
      "Paint",
      "Cook",
      "Bake",
      "Eat",
      "Drink",
      "Sleep",
      "Wake",
      "Work",
      "Study",
      "Learn",
      "Teach",
      "Listen",
      "Speak",
      "Think",
      "Dream",
      "Imagine",
      "Create",
      "Design",
      "Build",
      "Fix",
      "Repair",
      "Clean",
      "Wash",
      "Brush",
      "Comb",
      "Drive",
      "Ride",
      "Fly",
      "Sail",
      "Travel",
      "Explore",
      "Discover",
      "Admire",
      "Love",
      "Hug",
      "Kiss",
      "Touch",
      "Feel",
      "See",
      "Watch",
      "Play",
      "Win",
      "Lose",
      "Compete",
      "Challenge",
      "Help",
      "Support",
      "Encourage",
      "Motivate",
      "Inspire",
      "Guide",
      "Direct",
      "Lead",
      "Follow",
      "Collaborate",
      "Communicate",
      "Connect",
      "Network",
      "Celebrate",
      "Party",
      "Relax",
      "Enjoy",
      "Entertain",
      "Surprise",
      "Amaze",
      "Impress",
      "Express",
      "Share",
      "Donate",
      "Volunteer",
      "Protest",
      "Fight",
      "Protect",
      "Save",
      "Rescue",
      "Heal",
      "Cure",
      "Comfort",
      "Empower",
      "Transform",
      "Innovate",
      "Change"
    ),
    pronouns: listOf(
      "I",
      "you",
      "he",
      "she",
      "it",
      "we",
      "they",
      "me",
      "him",
      "her",
      "us",
      "them",
      "myself",
      "yourself",
      "himself",
      "herself",
      "itself",
      "ourselves",
      "themselves"
    ),
    adverbs: listOf(
      "Quickly",
      "Slowly",
      "Loudly",
      "Softly",
      "Suddenly",
      "Gradually",
      "Beautifully",
      "Ugly",
      "Carefully",
      "Carelessly",
      "Effortlessly",
      "Eagerly",
      "Reluctantly",
      "Honestly",
      "Dishonestly",
      "Clearly",
      "Unclearly",
      "Perfectly",
      "Imperfectly",
      "Regularly",
      "Irregularly",
      "Often",
      "Rarely",
      "Always",
      "Never",
      "Somewhere",
      "Everywhere",
      "Nowhere",
      "Here",
      "There",
      "Everywhere",
      "Anywhere",
      "Behind",
      "In front",
      "Upstairs",
      "Downstairs",
      "Inside",
      "Outside",
      "Above",
      "Below",
      "Near",
      "Far",
      "Close",
      "Distant",
      "High",
      "Low",
      "First",
      "Last",
      "Early",
      "Late",
      "Soon",
      "Now",
      "Later",
      "Yesterday",
      "Today",
      "Tomorrow",
      "Again",
      "Once",
      "Twice",
      "Thrice",
      "Together",
      "Apart",
      "Only",
      "Also",
      "Even",
      "Almost",
      "Nearly",
      "Hardly",
      "Scarcely",
      "Certainly",
      "Maybe",
      "Perhaps",
      "Possibly",
      "Truly",
      "False",
      "Really",
      "Actually",
      "Basically",
      "Naturally",
      "Normally",
      "Fortunately",
      "Unfortunately",
      "Honestly",
      "Frankly",
      "Generally",
      "Specifically",
      "Personally",
      "Publicly",
      "Privately",
      "Quietly",
      "Loudly",
      "Seriously",
      "Jokingly"
    ),
  },
};

class Colour {
  constructor(data) {
    this.name = data && data.name;
    this.hex = data && data.hex;
  }

  toHSL() {
    return this.toRGB().toHSL();
  }

  toCMYK() {
    return this.toRGB().toCMYK();
  }

  toRGB() {
    if (!this.hex.startsWith("#") || !this.hex.length === 7) return null;
    const [r1, g1, b1] = [
      this.hex.slice(1, 3),
      this.hex.slice(3, 5),
      this.hex.slice(5),
    ];
    const [r, g, b] = [
      parseInt(`0x${r1}`),
      parseInt(`0x${g1}`),
      parseInt(`0x${b1}`),
    ];
    return new RGBA(r, g, b, 1, this.name);
  }

  toHex() {
    return this.toRGB().hex;
  }
}

class RGBA extends Colour {
  /**
   *
   * @param {Number} r - 0-255
   * @param {Number} g - 0-255
   * @param {Number} b - 0-255
   * @param {Number} a - 0-255
   */
  constructor(r, g, b, a = 1, name) {
    super({ name });
    this.r = r;
    this.g = g;
    this.b = b;
    this.a = a;
    const zeroPadded = (val) =>
      val < 16 ? "0" + val.toString("16") : val.toString("16");
    const [r1, g1, b1] = [zeroPadded(r), zeroPadded(g), zeroPadded(b)];
    this.hex = `#${r1}${g1}${b1}`.toUpperCase();
  }

  toHSL() {
    const r1 = this.r / 255;
    const g1 = this.g / 255;
    const b1 = this.b / 255;
    const Cmax = Math.max(r1, g1, b1);
    const Cmin = Math.min(r1, g1, b1);
    const delta = Cmax - Cmin;
    const h =
      Cmax === 0
        ? 0
        : Cmax === r1
        ? 60 * (((g1 - b1) / delta) % 6)
        : Cmax === g1
        ? 60 * ((b1 - r1) / delta + 2)
        : Cmax === b1
        ? 60 * ((r1 - g1) / delta + 4)
        : 0;
    const l = (Cmax + Cmin) / 2;
    const s = delta === 0 ? 0 : delta / (1 - Math.abs(2 * l - 1));
    const toDegrees = (val) =>
      val < 0 ? Math.round(360 + val) : Math.round(val);
    return new HSL(
      toDegrees(h),
      Math.round(s * 100),
      Math.round(l * 100),
      this.name
    );
  }

  toCMYK() {
    const [r1, g1, b1] = [this.r / 255, this.g / 255, this.b / 255];
    const k = 1 - Math.max(r1, g1, b1);
    const c = (1 - r1 - k) / (1 - k);
    const m = (1 - g1 - k) / (1 - k);
    const y = (1 - b1 - k) / (1 - k);
    return new CMYK(
      Math.round(c * 100),
      Math.round(m * 100),
      Math.round(y * 100),
      Math.round(k * 100),
      this.name
    );
  }
}

class HSL extends Colour {
  /**
   *
   * @param {Number} h 0-360
   * @param {Number} s 0%-100%
   * @param {Number} l 0%-100%
   */
  constructor(h, s, l, name) {
    super({ name });
    this.h = h;
    this.s = s;
    this.l = l;
  }

  toRGB() {
    const s = this.s / 100;
    const l = this.l / 100;
    const C = (1 - Math.abs(2 * l - 1)) * s;
    const X = C * (1 - Math.abs(((this.h / 60) % 2) - 1));
    const m = l - C / 2;
    const [r1, g1, b1] = isBetween(this.h, 0, 60)
      ? [C, X, 0]
      : isBetween(this.h, 60, 120)
      ? [X, C, 0]
      : isBetween(this.h, 120, 180)
      ? [0, C, X]
      : isBetween(this.h, 180, 240)
      ? [0, X, C]
      : isBetween(this.h, 240, 300)
      ? [X, 0, C]
      : isBetween(this.h, 300, 360)
      ? [C, 0, X]
      : [0, 0, 0];
    return new RGBA(
      Math.round((r1 + m) * 255),
      Math.round((g1 + m) * 255),
      Math.round((b1 + m) * 255),
      undefined,
      this.name
    );
  }

  toCMYK() {
    return this.toRGB().toCMYK();
  }
}

class CMYK extends Colour {
  /**
   *
   * @param {Number} c 0%-100%
   * @param {Number} m 0%-100%
   * @param {Number} y 0%-100%
   * @param {Number} k 0%-100%
   */
  constructor(c, m, y, k, name) {
    super({ name });
    this.c = c;
    this.m = m;
    this.y = y;
    this.k = k;
  }

  toRGB() {
    const [c, m, y, k] = [
      this.c / 100,
      this.m / 100,
      this.y / 100,
      this.k / 100,
    ];
    const r = 255 * (1 - c) * (1 - k);
    const g = 255 * (1 - m) * (1 - k);
    const b = 255 * (1 - y) * (1 - k);
    return new RGBA(
      Math.round(r),
      Math.round(g),
      Math.round(b),
      undefined,
      this.name
    );
  }

  toHSL() {
    return this.toRGB().toHSL();
  }
}

const faker = {
  repeat: function (n, fn) {
    return Array(n)
      .fill(null)
      .map(() => fn());
  },
  first_name: function () {
    return data.first_names.female.concat(...data.first_names.male).random();
  },
  male: {
    first_name: function () {
      return data.first_names.male.random();
    },
    full_name: function () {
      return `${this.male.first_name()} ${this.last_name()}`;
    },
  },
  female: {
    first_name: function () {
      return data.first_names.female.random();
    },
    full_name: function () {
      return `${this.female.first_name()} ${this.last_name()}`;
    },
  },
  last_name: function () {
    return data.last_names.random();
  },
  full_name: function () {
    return `${this.first_name()} ${this.last_name()}`;
  },
  email: function (
    first_name = this.first_name(),
    last_name = this.last_name(),
    yearBlurb = randInt(0, 100)
  ) {
    return `${first_name}.${last_name}${yearBlurb}@${data.email_domains.random()}`.toLowerCase();
  },
  date: function (after = new Date(0), before = new Date()) {
    return new Date(randInt(after.getTime(), before.getTime()));
  },
  colour: function () {
    return new Colour(data.colours.random());
  },
  rgb: function () {
    const [r, g, b] = [randInt(0, 255), randInt(0, 255), randInt(0, 255)];
    return new RGBA(r, g, b, 1);
  },
  rgba: function () {
    const { r, g, b } = this.rgb();
    return new RGBA(r, g, b, parseFloat(Math.random().toFixed(2)));
  },
  hsl: function () {
    const [h, s, l] = [randInt(0, 360), randInt(0, 100), randInt(0, 100)];
    return new HSL(h, s, l);
  },
  hwb: function () {
    const [h, w, b] = [randInt(0, 360), randInt(0, 100), randInt(0, 100)];
    return { h, w, b };
  },
  cmyk: function () {
    const [c, m, y, k] = [
      randInt(0, 100),
      randInt(0, 100),
      randInt(0, 100),
      randInt(0, 100),
    ];
    return new CMYK(c, m, y, k);
  },
  address: function () {
    const street_number = randInt(1, 9999);
    const street_name = `${data.address.street_names.random()}${
      Math.random() > 0.6 ? ` ${data.cardinal_direction.random()}` : ""
    }`;
    const street_type = data.address.street_types.random();
    const city = data.cities.random();
    const state = data.states.random();
    const postcode = randInt(1000, 99999);
    return {
      street_number,
      street_name,
      street_type,
      city,
      state,
      postcode,
      string: `${street_number} ${street_name} ${street_type}, ${city}, ${state} ${postcode}`,
    };
  },
  user: function ({ sex = undefined, minAge = 18 } = {}) {
    const first_name = sex
      ? sex[0].toLowerCase() === "m"
        ? this.male.first_name()
        : this.female.first_name()
      : this.first_name();
    const last_name = this.last_name();
    const today = new Date();
    today.setFullYear(today.getFullYear() - minAge);
    const dob = this.date(undefined, today);
    const yearBlurb = Math.round(
      (dob.getFullYear() / 100 - parseInt(dob.getFullYear() / 100)) * 100
    );
    const email = this.email(first_name, last_name, yearBlurb);
    const phone_number = `${randInt(0, 999)} ${randInt(1000, 9999)} ${randInt(
      1000,
      9999
    )}`;
    return {
      first_name,
      last_name,
      email,
      phone_number,
      dob,
      address: this.address(),
    };
  },
};

module.exports = { faker };
